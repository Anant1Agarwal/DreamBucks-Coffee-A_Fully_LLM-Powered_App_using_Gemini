...
const ProductList:React.FC<ProductListProps> = ({ products, quantities, setQuantities,totalPrice }:ProductListProps) => {
    ...
    const renderItem = ({ item }: { item: Product }) => (
        <View className="flex-row items-center justify-between mx-7 pb-3">
          ...
          <View className="flex-row items-center">
            ...
            <Text className="mx-3 mt-1 font-[Sora-SemiBold]">{quantities[item.name] || 0}</Text>
            <TouchableOpacity onPress={() => setQuantities(item.name, 1)}>
              <Text className="text-xl">+</Text>
            </TouchableOpacity>
          </View>
